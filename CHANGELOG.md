...
## 2026-05-25 — Silent-account guest checkout (Option A)

### Feat — Player books without sign-up; silent profile + magic-link sign-in email

- **Migration `0032`**: `user_signup_method` enum (`password` | `guest_magic_link`), `profiles.signup_method` column (default `password`, backfilled), `venues.allow_guest_checkout boolean default true` (per-venue opt-out for owners burned by fraud), partial index `profiles_guest_phone_idx` on guest phones for future per-phone throttling.
- **`features/auth/guest.ts`** — single module owning guest resolution. `resolveOrCreateGuestPlayer({email,displayName,phoneE164})` is idempotent on email: calls `admin.auth.admin.createUser({ email_confirm: true, user_metadata })` then `db.insert(profiles).onConflictDoNothing({ target: profiles.id })`; race-safe re-read on conflict. `generateMagicSignInLink(email, redirectPath)` wraps `admin.auth.admin.generateLink({ type: 'magiclink' })` and returns `null` on failure so a flaky magic-link never blocks a saved booking.
- **`features/booking/actions.ts`** — added `resolvePlayerForBooking(form, venueSlug)` helper. Auth path uses existing per-user `bookingCreate` limiter. Guest path checks `venue.allowGuestCheckout` opt-in, parses a Zod `guestBlockSchema` (`displayName` 1–80, `email`, `phoneE164` `/^\+63\d{10}$/`), applies an **IP-keyed** `guestBookingCreate` limiter (5 / 10 min — harder than authenticated because each request silently creates an auth user), then calls `resolveOrCreateGuestPlayer`. Both `startBookingAction` and `startBookingReturningIdAction` use the helper; on guest fail with `guest_checkout_disabled` / `validation_failed` they fall back to the legacy `/sign-in?next=...` redirect so existing pickers keep working.
- **After-response email** — `notifyGuestBookingMagicLink(bookingId, { isNewAccount })` dispatched via `next/server` `after()` so the action returns immediately. Email tagged `guest_booking_magic_link`; copy varies by `isNewAccount`. CTA is the magic-link URL with `payUrl` fallback if `generateLink` returned null.
- **Booking-flow client** — `BookingFlow` now opens the details modal when `allowGuestCheckout && !isAuthenticated` instead of redirecting to `/sign-up`. Step 1 modal for guests: required name + email + phone (`+63\d{10}` pattern enforced both UI and server), **email-confirm field with `onPaste` prevention** for typo guard, brand-tinted "No account needed" banner. Footer copy switches to "No account needed · already have one? Sign in" when guest checkout is on. `proceedToPayment` appends `guestName/guestEmail/guestPhone` to the FormData when unauthenticated; the typed email also becomes `contactEmail` so booking confirmation reaches the guest inbox.
- **Owner venue editor** — new `GuestCheckoutSection` in `venue-form.tsx` with the hidden-then-checkbox pattern (`<input type="hidden" name="allowGuestCheckout" value="false">` + `<input type="checkbox" name="allowGuestCheckout" value="true">`) so `Object.fromEntries(form)` always sees the checkbox state. Zod transform: `(v) => v !== "false"` (default ON unless explicitly disabled). Service writes the field in `createVenue` + `updateVenue`.
- **Invariants** — service layer (`features/booking/service.ts`) stays `playerId`-based and unchanged: every guest booking has a real auth user + profile so `/me/bookings`, no-show tracking, refunds, ledger, RLS, and Supabase Realtime all keep working. `email_confirm: true` on `createUser` is acceptable verification because magic-link delivery itself proves inbox ownership before the player can manage the booking.
