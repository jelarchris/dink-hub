...
import { listLateConfirmSignupCandidates } from "@/features/open-play";
import { formatDateTimeManila } from "@/lib/date";
import { formatPHP } from "@/lib/money";
import { LateConfirmSignupForm } from "./late-confirm-form";

export const dynamic = "force-dynamic";
export const metadata = { title: "Admin · Late-confirm open-play signups" };

export default async function AdminLateConfirmSignupsPage() {
  const rows = await listLateConfirmSignupCandidates(100);
  ...
  return (
    <Container className="py-3 sm:py-4">
      <PageHeader
        ...
        kicker="Admin"
        title="Late-confirm open-play signups"
        subtitle="Open-play sessions that already ended but a player's payment is still in 'payment submitted'. Use this only when the owner is unreachable AND the player has documented evidence the session occurred."
      />
      ...
      {rows.length === 0 ? (
        <EmptyState
          icon={<ShieldCheck />}
          title="Nothing stuck"
          description="No past-end signups are awaiting verification."
        />
      ) : (
            ...
            return (
              <Card key={row.payment.id}>
                <CardContent className="space-y-4 p-5">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div>
                      <div className="font-semibold">
                        {row.venue.name} · {row.session.title}
                      </div>
                      <div className="text-sm text-[var(--color-fg-muted)]">
                        {formatDateTimeManila(row.session.startAt)} → {formatDateTimeManila(row.session.endAt)}
                      </div>
                    </div>
                    <div className="text-right text-sm">
                      <div className="font-semibold">{formatPHP(row.payment.amountCentavos)}</div>
                  ...
                  <LateConfirmSignupForm paymentId={row.payment.id} />
                </CardContent>
              </Card>
            );
          })}
