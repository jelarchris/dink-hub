...
// IMPORTANT: import server action DIRECTLY from the actions file. Going via
// the feature barrel pulls server-only modules into the client bundle.
import { submitInvoiceReceiptAction } from "@/features/owner-invoices/actions";
...
export function InvoiceReceiptForm({ invoiceId }: { invoiceId: string }) {
  ...
  if (success) {
    return (
      <Alert variant="success" icon={<Check />} title="Receipt uploaded">
        Waiting for DinkHub to verify. Refresh in a moment to see your status update.
      </Alert>
    );
  }
        ...
        {({ id, describedBy, invalid }) => (
          ...
          >
            ...
            <span className="font-medium">
              {fileName ?? "Tap to select your GCash receipt"}
            </span>
